...
class DistriConfig:
    ...
    def split_idx(self, rank: int or None = None) -> int:
        """Return index of the responsible data in current batch."""
        # TODO(MX): batch size > 1?
        if rank is None:
            rank = self.rank
...
class PatchParallelismCommManager:
    ...
    def create_buffer(self):
        # Called after at least one round of dummy run
        # In the pipeline
    ...
    def get_buffer_list(self, idx: int) -> list[torch.Tensor]:
        # Get all parts of target tensor from all other devices
        buffer_list = [t[self.starts[idx] : self.ends[idx]].view(self.shapes[idx]) for t in self.buffer_list]
        return buffer_list

    def communicate(self):
        distri_config = self.distri_config
        # If we have multiple com, they must be continuos
