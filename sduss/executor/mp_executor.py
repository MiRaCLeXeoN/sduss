...
async def async_event_wrapper(event):
    event.set()
...
class MpExecutor:
    def __init__(
        ...
        self.output_queue: 'multiprocessing.Queue[TaskOutput]' = multiprocessing.Queue(100)
    ...
    def _process_task_output(self, event_loop):
        while True:
            ...
            asyncio.run_coroutine_threadsafe(async_event_wrapper(event), event_loop)
    ...
    def end_worker(self):
        ...
        self.task_queue.put(Task(method_name="shutdown", need_res=True))
        self.process.join()
